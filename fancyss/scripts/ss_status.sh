...
SOCKS5_OPEN=$(netstat -nlp 2>/dev/null | grep -w "23456" | grep -Eo "ss-local|sslocal|v2ray|xray|trojan|naive|tuic|hysteria|clash")
REDIRC_OPEN=$(netstat -nlp 2>/dev/null|grep -w "3333"|grep -Eo "ss-redir|sslocal|v2ray|xray|trojan|ipt2socks|hysteria|clash")
...
get_china_status(){
	...
	if [ "${ret_code}" = "200" ] || [ "${ret_code}" = "204" ] || [ "${ret_code}" = "301" ] || [ "${ret_code}" = "302" ];then
	...
	if [ "${ret_code}" = "200" ] || [ "${ret_code}" = "204" ] || [ "${ret_code}" = "301" ] || [ "${ret_code}" = "302" ];then
		ret_time_ext=$(echo $ret0 | awk -F "|" '{printf "%.0f ms\n", $1 * 1000}')
		log2='国内链接 【'${LOGTIME}'】 ✓'
	else
